...
public class KriftoBabyAnimations {
    ...
    /**
     * Ported from {@code KriftoAnimations.tamed}. Rotations cross unchanged — every one of them is
     * bit-for-bit identical to the adult clip, the same relationship {@code walk} already has between
     * the two rigs for {@code right_leg1}/{@code left_leg1}. Y-position bounces (root hop, front-leg
     * pump, back-leg follow-through) are scaled ×0.5, the exact ratio {@code walk} already uses between
     * {@code gBody_parts} (1.0px) and {@code body_parts} (0.5px) for the same kind of cyclic bounce.
     *
     * <p>The adult's {@code gRight_calf}/{@code gLeft_calf} bounce is folded onto
     * {@code right_leg2}/{@code left_leg2} directly — this rig has no separate calf segment, the whole
     * back leg is one bone. {@code gRight_claws2}/{@code gLeft_claws2} has no bone to receive it at all
     * and is dropped; at this clip's scale the claw curl was never going to read on a chick anyway.
     */
